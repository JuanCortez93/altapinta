...
>
> 1. **Dinero** — la plata que hay (Caja chica + Cuenta Corriente + Reserva + Tesoro) coincide
...
| Cuentas de dinero | Cuatro: **Tesoro** (efectivo, la caja del lugar), **Caja chica** (efectivo operativo), **Cuenta Corriente** (Mercado Pago: adonde caen las transferencias del día) y **Reserva** (Mercado Pago: adonde se barre la Cuenta Corriente al final del día; genera interés). Ninguna de las dos de MP es efectivo. Sin cuenta de provisión de sueldos. |
| Ventas | **Efectivo:** se declara contando la caja al final del turno ("Efectivo contado al final"); la venta la calcula el sistema restando lo que ya había en Caja chica antes del turno. **Transferencia:** se declara directo (cae en la Cuenta Corriente de MP). Así coincide con cómo ya anotan en papel. |
...
| Arqueo Caja chica | **Ya no existe como paso aparte.** Al definir la venta a partir del conteo, la caja cierra por construcción. Si el conteo da menos que lo que ya había (imposible: venta negativa), el cierre se rechaza con un mensaje claro. El arqueo de la **Cuenta Corriente (MP)** sí sigue siendo una verificación real (opcional), contra lo que declara Mercado Pago antes de barrerla a la Reserva. |
| Mover dinero | Pantalla en **Cuentas**: pasar plata de una cuenta a otra (p. ej. cambiar transferencia por efectivo, o barrear Cuenta Corriente → Reserva al final del día). Categoría `conversion`, no es venta ni gasto. |
...
| Compras | **Multi-renglón** (se acabó cargar producto por producto). Catálogo de productos + "Otro" texto libre. Cada renglón lleva **presentación** cajón / kilo / unidad, sólo cuando el producto admite más de una. Genera `stock_lots` (salvo los "Otro") + un egreso de plata (Caja chica, Cuenta Corriente, Tesoro o Reserva). |
...
Durante el día   se cargan los gastos apenas ocurren (efectivo o
                 transferencia → egreso de Caja chica o Cuenta Corriente)
Cierre de turno  turno (mañana / tarde / domingo) + quién cierra
                 efectivo contado al final (→ calcula la venta en efectivo)
                 + vendido en transferencia (directo, a la Cuenta Corriente)
                 checklist de los gastos del día: se tildan los del turno
                 barrido Caja chica → Tesoro (normalmente en el cierre de tarde)
                 arqueo de la Cuenta Corriente (opcional, contra el saldo real)
Fin del día      barrido manual Cuenta Corriente → Reserva ("Mover dinero")
...
- **Cuentas:** cuatro — **Tesoro** (efectivo, la caja del lugar), **Caja chica**
  (efectivo operativo del día), **Cuenta Corriente** (MP: adonde van las
  transferencias del día) y **Reserva** (MP: adonde se barre la Cuenta
  Corriente al final del día). Ninguna de las dos de MP es efectivo.
...
> Las migraciones `0002` a `0006` están escritas a mano (drizzle-kit `generate`
...
> nuevo (siguiendo el patrón de `0002`-`0006`), copiar el último snapshot como
